...
import type { IPromotion } from "@/models/Promotions/promotion.model";
...
const PromotionsPage: React.FC = () => {
  ...
  const [itemSelected, setItemSelected] = useState<IPromotion>()
  const { register, handleSubmit, formState: { errors }, reset } = useForm<IPromotion>()
  ...
  const getData = useCallback(async (page?: number | undefined) => {
    try {
      ...
      const response = await PromotionsService.listPromotions(
        {
          page: page ? page : 1,
          text_search: '',
        }
  ...
  useLayoutEffect(() => {
    dispatch(changeTitle("Promociones"));
  }, [dispatch])
  ...
  const columns = [
    ...
    { Header: 'Fecha de inicio', accessor: 'start_date', cell: (value: string) => value ? new Date(value).toLocaleString() : '' },
    { Header: 'Fecha de fin', accessor: 'end_date', cell: (value: string) => value ? new Date(value).toLocaleString() : '' },
    { Header: 'Destacada', accessor: 'is_featured', cell: (value: boolean) => value ? 'Sí' : 'No' },
  ...
  const actions = [
    ...
    },
    {
      ...
      label: 'Eliminar',
      color: 'text-red-400',
      disabled: (row: IPromotion) => row.is_active === false,
      onClick: (row: IPromotion) => {
        onDelete(row)
      }
    },
  ...
  const onGetDetail = (row: IPromotion) => {
    ...
    reset({
      ...row,
      start_date: formatDateTimeLocal(row.start_date),
      end_date: formatDateTimeLocal(row.end_date),
    })
  }
  ...
  const onSubmit = async (data: IPromotion) => {
    try {
      ...
      if (selectedNewImage && notContentOldImage) {
        toast.error('Debes subir una imagen de la promoción')
        return
      }
      setStatusScreenDetail(ScreenStatus.loading)
      if (itemSelected) {
        if (!itemSelected.uuid) {
          toast.error('No se encontró el identificador de la promoción')
          setStatusScreenDetail(ScreenStatus.success)
          return
        }
        await PromotionsService.updatePromotion({ uuid_promotion: itemSelected.uuid, data })
  ...
  const onDelete = async (row: IPromotion) => {
    try {
      if (!row.uuid) {
        toast.error('No se encontró el identificador de la promoción')
        return
      ...
      const accepted = await confirm({
        ...
        content: (
          <span>
            Esto eliminará la promoción {row.name}
          </span>
        )
      })
      if (!accepted) return

      await PromotionsService.deletePromotion({ uuid_promotion: row.uuid })
  ...
  return (<>
    ...
    <TableComponent data={data.results}
      ...
      onSearch={onSearch}
      headerRightComponent={<Button type="button" onClick={onCreate}>Nueva Promoción</Button>}
    />

    <ModalComponent show={shoDetail} setShow={() => setShowDetail(false)}
      disableClose={false}
      statusModal={statusScreenDetail}
      title={itemSelected ? `Editar ${itemSelected.name}` : 'Crear nueva promoción'}
      ...
      children={
        <>
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 items-top">
            ...
            <FormInput label="Fecha de inicio"
              name="start_date"
              type="datetime-local"
              maxLength={80}
              error={errors.start_date} register={register('start_date',
                {
                  required: {
                    value: true,
            ...
            <FormInput label="Fecha de fin"
              name="end_date"
              type="datetime-local"
              maxLength={80}
              error={errors.end_date} register={register('end_date',
                {
                  required: {
                    value: true,
            ...
            <FormInput label="Destacada"
              name="is_featured"
              type="checkbox"
              error={errors.is_featured} register={register('is_featured')} />
